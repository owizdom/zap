...
struct ZapRecord {
    ...
    amount: u256,
    recipient_hash: felt252,   // keccak256(to_email) — for auditability
    created_at: u64,
    claimed: bool,
}
...
mod ZapVault {
    ...
    impl ZapVaultImpl of IZapVault<ContractState> {
        ...
        /// Sender deposits tokens into the vault for a given zap_id.
        ...
        fn deposit(
            ...
            erc20.transfer_from(sender, starknet::get_contract_address(), amount);
        ...
        /// Owner releases funds to the recipient.
        /// Called by the backend after the recipient authenticates.
        fn release(
            ref self: ContractState,
            zap_id: felt252,
            recipient: ContractAddress,
        ...
        /// Sender reclaims funds after 30-day expiry.
        fn refund(ref self: ContractState, zap_id: felt252) {
            ...
            let caller = get_caller_address();
            assert(caller == zap.sender, 'ZapVault: not sender');

            let now = get_block_timestamp();
            assert(now >= zap.created_at + EXPIRY_SECONDS, 'ZapVault: not yet expired');
